...
# -------------------------------------------------------------
# Paso 4: Mostrar y guardar resultados de rendimiento
# -------------------------------------------------------------
...
sorted_by_performance = df_results.sort_values(by='Test R²', ascending=False)['Model'].tolist()
model_to_rank = {model: idx + 1 for idx, model in enumerate(sorted_by_performance)}

COLOR_MAP = {
    1: "\033[1;92m",      # Verde claro (ex-segundo)
    2: "\033[1;32m",      # Verde oscuro (ex-primero)
    3: "\033[1;93m",      # Amarillo
    4: "\033[38;5;208m",   # Naranja
    5: "\033[1;31m",      # Rojo
}
RESET = "\033[0m"
...
width = len(lines[0])
print("\n" + "="*width)
print("                           COMPARATIVA DE RENDIMIENTO DE MODELOS".center(width))
print("="*width)
print(lines[0])
...
print(f"Resultados de rendimiento guardados en: {results_csv_path}")

# Guardar resultados en un archivo JSON compartido
...
with open(json_path, 'w', encoding='utf-8') as f:
    json.dump(list(merged_dict.values()), f, indent=4, ensure_ascii=False)
print(f"Métricas actualizadas en el archivo JSON: {json_path}")

# -------------------------------------------------------------
# Paso 5: Analizar y guardar coeficientes
# -------------------------------------------------------------
# El modelo Dummy no tiene coeficientes, por lo que extraemos coeficientes para LR, Lasso y Ridge
coefs = pd.DataFrame({
    'Feature': X_train.columns,
    'Linear Regression': models['Linear Regression'].coef_,
    'Lasso (alpha=1)': models['Lasso (alpha=1)'].coef_,
    'Ridge (alpha=1)': models['Ridge (alpha=1)'].coef_
})
...
# Interceptores
lr_intercept = models['Linear Regression'].intercept_
lasso_intercept = models['Lasso (alpha=1)'].intercept_
ridge_intercept = models['Ridge (alpha=1)'].intercept_
print(f"Interceptores:\n - Linear Regression: {lr_intercept:.4f}\n - Lasso (alpha=1): {lasso_intercept:.4f}\n - Ridge (alpha=1): {ridge_intercept:.4f}\n")

coefs_csv_path = os.path.join(output_dir, 'model_coefficients.csv')
coefs.to_csv(coefs_csv_path, index=False)
print(f"Coeficientes guardados en: {coefs_csv_path}")
